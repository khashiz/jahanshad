...
else
{
	if ($this->canReply)
	{
		...
		{
			?>
			<div class="uk-width-1-1"><button type="button" class="uk-button uk-button-primary uk-button-large uk-border-rounded uk-box-shadow-small uk-width-1-1 uk-flex-center" id="com-rsticketspro-reply-button" onclick="RSTicketsPro.showReply(this);"><i class="fas fa-plus"></i><span><?php echo JText::_('RST_TICKET_REPLY'); ?></span></button></div>
			<?php
		}
		?>
		<div id="com-rsticketspro-reply-box" <?php if (!$showReply) { ?>class="hidden"<?php } ?>>
			...
			?>
			<div class="uk-margin-top"><button type="button" onclick="Joomla.submitbutton('ticket.reply')" class="uk-button uk-button-primary uk-button-large uk-border-rounded uk-box-shadow-small"><i class="far fa-send fa-flip-horizontal"></i><span><?php echo JText::_('RST_TICKET_SUBMIT'); ?></span></button></div>
        </div>
		<?php
	}
}