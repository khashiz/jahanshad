        ...
        <div class="uk-child-width-1-2 uk-child-width-auto@s uk-grid-small" id="ticket-buttons" data-uk-grid>
            ...
            if (!$this->isPrint)
            {
                ...
                if ($this->ticket->status_id == RST_STATUS_CLOSED && $this->canOpenTicket)
                {
                    ?>
                        <div>
                            <a href="<?php echo JRoute::_('index.php?option=com_rsticketspro&task=ticket.reopen&id='.$this->ticket->id); ?>" class="uk-button uk-button-primary uk-border-rounded uk-box-shadow-small uk-width-1-1"><i class="fas fa-refresh"></i><span><?php echo JText::_('RST_TICKET_OPEN'); ?></span></a>
                        </div>
                    <?php
                }
                elseif ($this->ticket->status_id != RST_STATUS_CLOSED && $this->canCloseTicket)
                {
                    ?>
                        <div>
                            <a href="<?php echo JRoute::_('index.php?option=com_rsticketspro&task=ticket.close&id='.$this->ticket->id); ?>" class="uk-button uk-button-danger uk-border-rounded uk-box-shadow-small uk-width-1-1"><i class="fas fa-close"></i><span><?php echo JText::_('RST_TICKET_CLOSE'); ?></span></a>
                        </div>
                    <?php
                }
            }
                ...
                <?php if (is_null($user)) { ?>
                    <div>
                        <div class="uk-alert uk-alert-primary uk-padding-small uk-border-rounded">
                            <p class="uk-text-center uk-text-tiny font f700"><?php echo '<span class="f700">'.$this->showDate($message->date).'</span> ، '.strip_tags(RSTicketsProHelper::showMessage($message),'<span>'); ?></p>
                        </div>
                        ...
                        <div class="uk-grid-small" data-uk-grid>
                            ...
                            <div class="uk-width-2-3">
                                <div class="uk-background-muted uk-border-rounded uk-padding-small">
                                    <div class="font uk-text-tiny uk-margin-small-bottom f500 uk-text-muted ss02"><?php echo JText::sprintf('TICKET_INFO', '<span class="uk-display-inline-block f500">'.$text.'</span>', '<span class="uk-display-inline-block f500">'.$this->showDate($message->date).'</span>') ?></div>
