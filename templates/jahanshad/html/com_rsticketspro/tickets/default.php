	...
	<tbody>
		...
		foreach ($this->items as $i => $item)
		{
			...
			<tr class="rst_priority_color_<?php echo $item->priority_id; ?>">
				...
				<td class="uk-text-small font f700 uk-text-secondary uk-text-center fnum"><?php echo '<span class="uk-hidden@s uk-text-muted">'.JText::_('RST_TICKET_DATE').'&ensp;:&ensp;</span>'.$this->escape($this->showDate($item->date)); ?></td>
				<?php } ?>

				<?php if ($this->params->get('show_last_reply', 1)) { ?>
				<td class="uk-text-small font f700 uk-text-secondary uk-text-center fnum"><?php echo '<span class="uk-hidden@s uk-text-muted">'.JText::_('RST_TICKET_LAST_REPLY').'&ensp;:&ensp;</span>'.$this->escape($this->showDate($item->last_reply)); ?></td>
				<?php } ?>
        ...
        <?php } else { ?>
            <div>
                <div class="uk-placeholder uk-margin-remove uk-text-center uk-padding-large">
                    <div class="uk-margin-bottom"><i class="far fa-5x fa-message-lines uk-text-muted"></i></div>
